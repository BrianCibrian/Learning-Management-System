...
public class ViewFirstAdmin {
	...
	//Student note: username requirement labels
	...
	//Student note: password requirement labels
	...
	private ViewFirstAdmin() {
		...
		// Label to display the welcome message for the first user
		setupLabelUI(label_TitleLine2, "Arial", 18, width, Pos.CENTER, 0, 120);
		...
		setupButtonUI(button_AdminSetup, "Dialog", 18, 250, Pos.CENTER, 300, 560);
		...
		// Student Note: labels for username and password requriements 
		...
		theRootPane.getChildren().addAll(label_ApplicationTitle, label_TitleLine1,
				...
				button_Quit,label_UNameRequirements, label_UStart, label_UAllowed, label_USpecialFollow, label_ULength,
				label_Requirements, label_UpperCase, label_LowerCase, label_NumericDigit, label_SpecialChar,
			    label_LongEnough, label_NotTooLong, validPassword, label_PasswordsMatch);

		//Student Note: default state for username and password 
	...
	//Student Note: updates setup button 
	protected static void updateUserSetupButtonState() {
		String username = text_AdminUsername.getText();
		if (username == null) username = "";
		username = username.trim();

		String p1 = text_AdminPassword1.getText();
		if (p1 == null) p1 = "";
		String p2 = text_AdminPassword2.getText();
		if (p2 == null) p2 = "";

		// checks for roles and if passwords are equal
		boolean basicOk = p1.equals(p2);
		...
		String userErr = guiNewAccount.UsernameValidator.checkForValidUserName(username);
		if (!userErr.isEmpty()) {
			button_AdminSetup.setDisable(true);
			return;
		}
		...
		String passErr = guiNewAccount.PasswordValidator.evaluatePassword(p1);
		if (!passErr.isEmpty()) {
			button_AdminSetup.setDisable(true);
			return;
		}
		...
		// everything is valid
		button_AdminSetup.setDisable(false);
	}
	...
	static protected void updateUsernameAssessments() {
		String u = text_AdminUsername.getText();
		if (u == null) u = "";
	...
	static protected void updatePasswordAssessments() {
		String p = text_AdminPassword1.getText();
		if (p == null) p = "";
		
		String err = guiNewAccount.PasswordValidator.evaluatePassword(p);
		
		if (guiNewAccount.PasswordValidator.foundUpperCase) {
		...
		if (guiNewAccount.PasswordValidator.foundLowerCase) {
		...
		if (guiNewAccount.PasswordValidator.foundNumericDigit) {
		...
		if (guiNewAccount.PasswordValidator.foundSpecialChar) {
		...
		if (guiNewAccount.PasswordValidator.foundLongEnough) {
		...
		if (guiNewAccount.PasswordValidator.notTooLong) {
