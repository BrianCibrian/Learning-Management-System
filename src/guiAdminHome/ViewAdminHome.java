...
// Omar Note - next 3 added for invites
...
public class ViewAdminHome {
	...
	*/
	...
	protected static MenuButton menu_SelectRoles = new MenuButton("Select Roles"); // Omar change
	...
	// Omar HW3: added button for view post
	...
	protected static Button button_FeedbackSystem = new Button("Private Feedback");
	
	protected static Label label_OtpStatus = new Label(); //Chuan Nguyen added
	...
	*/
	...
	public static void displayAdminHome(Stage ps, User user) {
		...
		for (MenuItem mi : menu_SelectRoles.getItems()) {
		...
		// Call method to Update user count shown on admin home page
		updateUserCount();
	}
	...
	private ViewAdminHome() {
		...
		//Omar change for invites
		//setupComboBoxUI(combobox_SelectRole, "Dialog", 16, 90, 500, 205);
		for (String r : roles) {
		    CheckMenuItem item = new CheckMenuItem(r);
		    menu_SelectRoles.getItems().add(item);
		}
		menu_SelectRoles.setPrefWidth(120);
		menu_SelectRoles.setLayoutX(500);
		menu_SelectRoles.setLayoutY(210);
        ...
        // Chuan NEW
        setupButtonUI(button_TicketSystem, "Dialog", 16, 250, Pos.CENTER, 300, 270);
        button_TicketSystem.setOnAction((event) -> {
            ControllerAdminHome.openTicketSystem();
        });

        // Grading System button 
        setupButtonUI(button_GradingSystem, "Dialog", 16, 250, Pos.CENTER, 300, 320);
        button_GradingSystem.setOnAction((event) -> { ControllerAdminHome.openGradingSystem(); });

        // Brian NEW: Feedback System Button
        setupButtonUI(button_FeedbackSystem, "Dialog", 16, 250, Pos.CENTER, 300, 370);
        button_FeedbackSystem.setOnAction((event) -> { ControllerAdminHome.openFeedbackSystem(); });

		
		//Chuan added
		setupLabelUI(label_OtpStatus, "Arial", 16, width, Pos.BASELINE_LEFT, 300, 325);
		label_OtpStatus.setStyle("-fx-text-fill: red;");
		//label_OtpStatus.setStyle("");
		theRootPane.getChildren().add(label_OtpStatus);
		...
		// Omar Note - changed to have menu_SelectRoles roles and remove button_ManageInvitations button for now
		theRootPane.getChildren().addAll(
    		...
    		menu_SelectRoles, button_SendInvitation, line_Separator3,
    		...
    		button_ViewPosts,
    		button_TicketSystem, 
            button_GradingSystem, 
            button_FeedbackSystem  
    		);
	...
	*/
	...
	// Update user count shown on admin home page
	public static void updateUserCount() {
		int count = theDatabase.getNumberOfUsers();
		label_NumberOfUsers.setText("Number of users: " + count);
	}
}
