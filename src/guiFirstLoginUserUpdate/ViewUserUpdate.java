...
import guiNewAccount.PasswordValidator; // Chuan Nguyen added this
...
public class ViewUserUpdate {
	...
	// Omar note - log out first admin
	// This button enables the user to finish working on this page and proceed to the logout
	// page determined by the user's role at the time of log in.
	private static Button button_Logout = new Button("Proceed back to login");
	...
	private static TextInputDialog dialogUpdatePassword; // I aded
	private static TextInputDialog dialogConfirmPassword; //I added
	...
	private ViewUserUpdate() {
		...
		dialogUpdatePassword = new TextInputDialog(""); //Chuan Nguyen added
		dialogConfirmPassword = new TextInputDialog(""); //Chuan Nguyen added
		...
		dialogUpdatePassword.setTitle("Update Password"); //I added
		dialogUpdatePassword.setHeaderText("Enter your new password"); //I added
		
		dialogConfirmPassword.setTitle("Confirm Password"); //I added
		dialogConfirmPassword.setHeaderText("Re-enter your new password");//I added
        ...
        button_UpdatePassword.setOnAction((event) -> { //Chuan Nguyen added
            // Ask for new password
            Optional<String> r1 = dialogUpdatePassword.showAndWait();
            if (r1.isEmpty()) return;
            String p1 = r1.get().trim();
            if (p1.isEmpty()) {
            ...
            // Confirmation
            Optional<String> r2 = dialogConfirmPassword.showAndWait();
            if (r2.isEmpty()) return;
            String p2 = r2.get().trim();

            // Password Matching Conditional
            if (!p1.equals(p2)) {
                label_CurrentPassword.setText("Passwords do not match.");
                return;
            }
            //Password validation from guiNewAccount
            String message = PasswordValidator.evaluatePassword(p1);
            String summarizedReq = "At least one upper case letter\n"
            ...
            if (message != null && !message.isEmpty()) {
            	...
            	alert.setContentText(summarizedReq);
            ...
            theDatabase.updateUserPassword(theUser.getUserName(), p1);

            // Shows on screen change
            label_CurrentPassword.setText(p1);
        ...
        // Omar Note- change proceed to home page button to logout button
        setupButtonUI(button_Logout, "Dialog", 18, 300, 
        		Pos.CENTER, width/2-150, 450);
        button_Logout.setOnAction((event) -> {ControllerUserUpdate.performLogout(); });
    	
        // Populate the Pane's list of children widgets
        // Omar Note - change homepage button to logout button
